...
//    }
    ...
    //필드검증
    if (100 < addReq.getCount()) {
      bindingResult.rejectValue("count", null, "상품수량은 100개 초과할 수 없음");
    }
    //오브젝트 검증
    if (10_000_000 < addReq.getCount() * addReq.getPrice()) {
      bindingResult.reject(null, "총액(수량*단가)은 1000만원을 초과할 수 없습니다.");
    }
    if(bindingResult.hasErrors()){
      log.info("bindingResult={}",bindingResult);
      return ApiResponse.createApiResMsg("99", "실패", getErrMsg(bindingResult));
...
//    }
    ...
    //비즈니스 규칙
    //필드검증
    if(editReq.getCount() > 100){
      bindingResult.rejectValue("count",null,"상품수량은 100개를 초과할 수 없습니다.");
    }
    //오브젝트검증
    if(editReq.getCount() * editReq.getPrice() > 10_000_000){
